# Contributing to `mysql-proxy-operator`

Do you want to contribute to the `mysql-proxy-operator` repository? You've come to the right place then!
...
you can use this guide for specific information on contributing to the `mysql-proxy-operator` repository.
...
## Hacking on `mysql-proxy-operator`
...
### Before opening a pull request on the `mysql-proxy-operator` repository
...
By contributing to `mysql-proxy-operator`, you agree to license your contribution under
the Apache License 2.0 license.

### Adding a new file to `mysql-proxy-operator`
...
### Updating an existing file in `mysql-proxy-operator`
